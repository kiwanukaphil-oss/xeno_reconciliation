...
const FundUpload = () => {
  ...
  const fetchBatches = async () => {
    try {
      setLoading(true);
      const result = await getAllBatches(1, 50); // Fetch first 50 batches
              ...
              <div>
                ...
                <div className="flex gap-4 mt-2 text-sm">
                  <span className="text-gray-600">
                    Batch: {selectedBatchSummary.batchNumber}
                  </span>
                  <span className="text-gray-600">
                    Status: <span className={`font-semibold ${selectedBatchSummary.processingStatus === 'COMPLETED' ? 'text-green-600' : 'text-red-600'}`}>
                  ...
                  <div className="overflow-x-auto border border-red-200 rounded-lg">
                    <table className="min-w-full divide-y divide-red-200">
                      ...
                      <tbody className="bg-white divide-y divide-red-100">
                        {getCriticalErrors().slice(0, 50).map((error: any, index) => (
                  ...
                  <div className="overflow-x-auto border border-yellow-200 rounded-lg">
                    <table className="min-w-full divide-y divide-yellow-200">
                      ...
                      <tbody className="bg-white divide-y divide-yellow-100">
                        {getWarnings().slice(0, 100).map((warning: any, index) => (
               ...
               normalizeValidationData(selectedBatchSummary.validationWarnings).length > 0 && (
                <div className="mb-6">
                  <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
                    ...
                    Validation Warnings ({normalizeValidationData(selectedBatchSummary.validationWarnings).length})
                  </h3>
                    ...
                    <div className="space-y-2">
                      {normalizeValidationData(selectedBatchSummary.validationWarnings)
                        ...
                        .map((warning: any, idx: number) => (
                          <div key={idx} className="text-sm border-b border-yellow-200 pb-2 last:border-b-0">
                      ...
                      {normalizeValidationData(selectedBatchSummary.validationWarnings).length > 10 && (
                        <div className="text-xs text-yellow-700 text-center pt-2">
                          + {normalizeValidationData(selectedBatchSummary.validationWarnings).length - 10} more warnings...
                        </div>
                      )}
                    </div>
                  </div>
                   ...
                   newEntitiesData.newEntitiesReport.clients.length > 0 && (
                    <div>
                      ...
                      <div className="bg-gray-50 rounded p-3 space-y-1 text-sm">
                        {newEntitiesData.newEntitiesReport.clients.map((client: any, idx: number) => (
                  ...
                  {newEntitiesData.newEntitiesReport.accounts &&
                   newEntitiesData.newEntitiesReport.accounts.length > 0 && (
                    <div>
                      ...
                      <div className="bg-gray-50 rounded p-3 space-y-1 text-sm">
                        {newEntitiesData.newEntitiesReport.accounts.map((account: any, idx: number) => (
                          <div key={idx} className="flex justify-between">
